...
export default function ContactsPage() {
  ...
  const fetchAccountOptions = async (q: string): Promise<AccountOption[]> => {
    const res = await fetch('/api/accounts')
    const data = await res.json()
    const all: AccountOption[] = (data || []).map((a: any) => ({ value: a.id, label: a.name }))
    if (!q) return all.slice(0, 20)
    const needle = q.toLowerCase()
    return all.filter((o) => o.label.toLowerCase().includes(needle)).slice(0, 20)
  }
  ...
  const filteredContacts = contacts.filter((contact) => {
    ...
    const matchesAccount = !accountFilter || (contact as any).accountId === accountFilter.value
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-[1fr,260px] gap-4 mb-6">

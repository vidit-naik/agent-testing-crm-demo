'use client'

import { useState, useEffect } from 'react'
import dynamic from 'next/dynamic'
import { Modal, ModalFooter } from '../ui/Modal'
import { Input } from '../ui/Input'
import { Button } from '../ui/Button'
import { PortalSelect } from '../ui/PortalSelect'
import { useApp } from '@/contexts/AppContext'

const Select = dynamic(() => import('react-select').then((m) => m.default), { ssr: false }) as any

const STAGES = [
  'Prospecting',
  'Qualification',
  'Discovery',
  'Proposal',
  'Negotiation',
  'Closed Won',
  'Closed Lost',
]

export function EditOpportunityModal() {
  const { closeModal, addToast, modalState, triggerRefresh } = useApp()
  const [loading, setLoading] = useState(false)
  const [accounts, setAccounts] = useState<{ id: string; name: string }[]>([])
  const [formData, setFormData] = useState({
    name: '',
    account_id: '',
    stage: 'Prospecting',
    value: '',
    probability: '20',
    close_date: '',
    next_steps: '',
    owner: '',
  })

  useEffect(() => {
    fetch('/api/accounts')
      .then((r) => r.json())
      .then((d) => setAccounts(d || []))
      .catch(() => {})

    if (modalState.data?.opportunity) {
      const opp = modalState.data.opportunity
      setFormData({
        name: opp.name || '',
        account_id: opp.accountId || '',
        stage: opp.stage || 'Prospecting',
        value: opp.value?.toString() || '',
        probability: opp.probability?.toString() || '20',
        close_date: opp.closeDate ? new Date(opp.closeDate).toISOString().split('T')[0] : '',
        next_steps: opp.nextSteps || '',
        owner: opp.owner || '',
      })
    }
  }, [modalState.data])

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    setLoading(true)
    try {
      const res = await fetch(`/api/opportunities/${modalState.data?.opportunity?.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...formData,
          value: formData.value ? parseFloat(formData.value) : null,
          probability: parseInt(formData.probability),
        }),
      })
      if (!res.ok) throw new Error('Failed to update')
      addToast('Opportunity updated', 'success')
      closeModal()
      triggerRefresh()
    } catch {
      addToast('Failed to update opportunity', 'error')
    } finally {
      setLoading(false)
    }
  }

  const accountOptions = accounts.map((a) => ({ value: a.id, label: a.name }))
  const selectedAccount = accountOptions.find((o) => o.value === formData.account_id) || null

  return (
    <Modal isOpen onClose={closeModal} title="Edit Opportunity" size="lg">
      <form onSubmit={handleSubmit}>
        <div className="space-y-4">
          <Input
            label="Opportunity Name"
            required
            value={formData.name}
            onChange={(e) => setFormData({ ...formData, name: e.target.value })}
          />

          <div className="space-y-1">
            <label className="block text-sm font-medium">Account</label>
            <Select
              classNamePrefix="rs"
              value={selectedAccount}
              onChange={(opt: any) => setFormData({ ...formData, account_id: opt?.value || '' })}
              options={accountOptions}
              menuPortalTarget={typeof document !== 'undefined' ? document.body : undefined}
              styles={{ menuPortal: (base: any) => ({ ...base, zIndex: 2000 }) }}
              placeholder="Select an account"
              isClearable
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="block text-sm font-medium">Stage</label>
              <PortalSelect
                value={formData.stage}
                onChange={(v) => setFormData({ ...formData, stage: v })}
                options={STAGES.map((s) => ({ value: s, label: s }))}
              />
            </div>
            <Input
              label="Value"
              type="number"
              value={formData.value}
              onChange={(e) => setFormData({ ...formData, value: e.target.value })}
            />
          </div>

          <div className="grid grid-cols-2 gap-4">
            <Input
              label="Probability (%)"
              type="number"
              min="0"
              max="100"
              value={formData.probability}
              onChange={(e) => setFormData({ ...formData, probability: e.target.value })}
            />
            <Input
              label="Close Date"
              type="date"
              value={formData.close_date}
              onChange={(e) => setFormData({ ...formData, close_date: e.target.value })}
            />
          </div>

          <Input
            label="Owner"
            value={formData.owner}
            onChange={(e) => setFormData({ ...formData, owner: e.target.value })}
          />
          <Input
            label="Next Steps"
            value={formData.next_steps}
            onChange={(e) => setFormData({ ...formData, next_steps: e.target.value })}
          />
        </div>

        <ModalFooter>
          <Button type="button" variant="outline" onClick={closeModal} disabled={loading}>
            Cancel
          </Button>
          <Button type="submit" disabled={loading}>
            {loading ? 'Updating...' : 'Update Opportunity'}
          </Button>
        </ModalFooter>
      </form>
    </Modal>
  )
}
